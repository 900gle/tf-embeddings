...
def handle_query():
    ...
    with open(MULTI) as index_file:
        script_query_c = index_file.read().strip()
        script_query_c = script_query_c.replace("{query_vector}", str(query_vector))
    ...
    fig, ax = plt.subplots()
    ax.set_title('ANN vs kNN vs Multi ANN')
    line1, = ax.plot(t, score_a, lw=2, label='ANN')
    line2, = ax.plot(t, score_b, lw=2, label='kNN')
    line3, = ax.plot(t, score_c, lw=2, label='Multi')
...
if __name__ == '__main__':
    ...
    MULTI = "ann/query/multi_query.json"
